...
with lib; let
  ...
  buildNativeLibraries = artifacts:
    ...
    in
      runCommand "native" {buildInputs = [unzip];} ''
        mkdir -p $out/lib
        unzip ${zip} -d $out/lib
  ...
  buildAssets = versionInfo: assetsIndex:
    runCommand "assets" {} ''
      ${concatStringsSep "\n" (builtins.attrValues
        (flip mapAttrs assetsIndex.objects (name: a: let
          asset = fetchurl {
            sha1 = a.hash;
  ...
  buildBasicModule = versionInfo: assetsIndex: let
  ...
  buildFabricModules = versionInfo: assetsIndex: fabricProfile: let
    loaderVersion = fabricProfile.loader;
    loader = fabricLoaders.${loaderVersion};
    extraJavaLibraries =
      buildFabricLibraries
